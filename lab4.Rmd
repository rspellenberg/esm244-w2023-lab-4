...
surplus <- Fish_data %>% 
  ...
  drop_na() %>% 
  mutate(f_biomass = lead(TBbest)) %>%  ### f_biomass = future biomass
...
control_nls = nls(surplus ~ fox(m, carry, TBbest), 
                    ...
                    start = list(m = guess_vec[1], 
                                 carry = guess_vec[2]),
                    trace = TRUE,
                  control = nls.control(tol = 2.1e-9, maxiter = 1000)) 
### maxiter tells it how many times to run
...
# *come back to the lab key and compare if NLS out performs random model*
...
```{r}
### run this first "legend" line first and then the for loop
